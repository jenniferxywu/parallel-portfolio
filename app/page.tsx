...
type Portfolio = {
  ...
  dataQuality: string;
  funding: Array<{ accountId: string; currency: string; amount: number; status: string }>;
};
...
export default function Home() {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/portfolio", { cache: "no-store" }).then((response) => response.ok ? response.json() : null).then((data) => {
  ...
  const fundingLabel = (accountId: string) => {
    const rows = portfolio.funding.filter((item) => item.accountId === accountId);
    if (!rows.length) return "Funding history unavailable";
    return `Known funding ${rows.map((item) => `${item.currency} ${item.amount.toLocaleString(undefined, { maximumFractionDigits: 2 })}`).join(" + ")} · ${rows.some((item) => item.status !== "COMPLETE") ? "partial" : "reconciled"}`;
  };
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <div><h1>{activeView}</h1><p>Your tracked assets, without counting transfers as gains.</p></div>
        ...
        <div className="content">
          <section className="hero-grid">
            <div className="value-panel">
              <div className="eyebrow">TRACKED ASSET VALUE · {portfolio.dataQuality} <button aria-label="Hide value" onClick={() => setHidden(!hidden)}>◉</button></div>
              <div className="total-value">{money(portfolio.totalValue, portfolio.currency, hidden)}</div>
              <div className="performance">{portfolio.dayChange == null || portfolio.dayChangePct == null ? <><span>N/A</span><small>dated history incomplete</small></> : <><span>{signed(portfolio.dayChange, portfolio.currency)}</span><span>{portfolio.dayChangePct >= 0 ? "+" : ""}{portfolio.dayChangePct.toFixed(2)}%</span><small>today</small></>}</div>
              {portfolio.history.length ? <><div className="chart" aria-label="Portfolio value trend">{portfolio.history.map((point, index) => <i key={index} style={{ height: `${Math.max(16, point)}%` }} />)}</div><div className="chart-labels"><span>HISTORY</span><span>TODAY</span></div></> : <div className="chart chart-empty"><span>Historical valuation starts with reconciled snapshots</span></div>}
            </div>
            <div className="summary-panel">
              <div className="summary-row"><span>Non-cash assets</span><strong>{money(portfolio.invested, portfolio.currency, hidden)}</strong></div>
              <div className="summary-row"><span>Cash &amp; stablecoins</span><strong>{money(portfolio.cash, portfolio.currency, hidden)}</strong></div>
          ...
          <section className="section-block">
            ...
            <div className="account-grid">
              <article className="account-card moomoo-card">
                ...
                <div className="account-value">{money(portfolio.sources.Moomoo.value, portfolio.currency, hidden)}</div><small>{moomooShare}% of tracked assets · {fundingLabel("moomoo")}</small>
              ...
              <article className="account-card bitget-card">
                ...
                <div className="account-value">{money(portfolio.sources.Bitget.value, portfolio.currency, hidden)}</div><small>{100 - moomooShare}% of tracked assets · {fundingLabel("bitget")}</small>
          ...
          <section className="section-block holdings-block">
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>Asset</th><th>Platform</th><th>Holdings</th><th>Value</th><th>Allocation</th><th>Unrealized P&amp;L</th></tr></thead>
                <tbody>{holdings.map((item) => (
                  <tr key={`${item.source}-${item.symbol}`}>
                    ...
                    <td className={item.pnl == null ? "" : item.pnl >= 0 ? "positive" : "negative"}>{item.pnl == null || item.pnlPct == null ? <strong>N/A</strong> : <><strong>{item.pnl >= 0 ? "+" : "−"}{money(Math.abs(item.pnl), portfolio.currency, hidden)}</strong><small>{item.pnl >= 0 ? "+" : "−"}{Math.abs(item.pnlPct).toFixed(2)}%</small></>}</td>
                  </tr>
      ...
      {setupOpen && <div className="modal-backdrop" role="presentation">
