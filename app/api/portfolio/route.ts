...
async function getBitget(): Promise<{holdings:Holding[];total:number}> {
  ...
  const path="/api/v3/account/assets", timestamp=Date.now().toString();
  const response=await fetch(`https://api.bitget.com${path}`,{headers:{"ACCESS-KEY":apiKey,"ACCESS-SIGN":await signBitget(timestamp,"GET",path,secret),"ACCESS-TIMESTAMP":timestamp,"ACCESS-PASSPHRASE":passphrase,"Content-Type":"application/json","locale":"en-US"}});
  ...
  if(!response.ok || payload.code!=="00000" || !payload.data) throw new Error(payload.msg || "Bitget connection failed");
